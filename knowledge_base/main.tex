...
\include{ch02_exp_log}
\include{ch03_calculus1}
\include{ch04_calculus2}
\include{ch05_linear_algebra}
\include{ch06_probability}

%% ── Part II: Modelling financial data ────────────────────────────────────
%% Chapter 7 (Statistics) is planned. Its number is reserved by advancing the
%% counter, so every cross-reference stays valid once it is written.
\setcounter{chapter}{7}
\include{ch08_time_series}
\include{ch09_heavy_tails}

%% ── Part III: Generative modelling and evaluation ────────────────────────
%% Chapters 10-12 reserved (Optimisation, Neural Networks, Generative Models).
\setcounter{chapter}{12}
\include{ch13_gans}
%% Chapter 14 reserved (Financial Generative Modelling).
\setcounter{chapter}{14}
\include{ch15_evaluation}
\include{ch16_downstream}
...
\node[todo] (c7) at (0,-3.6)   {7. Statistics};
\node[node] (c8) at (4,-3.6)   {8. Time Series};
\node[node] (c9) at (8,-3.6)   {9. Heavy Tails};
\node[todo] (c10) at (8,-5.4)  {10. Optimisation};
\node[todo] (c11) at (4,-5.4)  {11. Neural Networks};
\node[todo] (c12) at (0,-5.4)  {12. Generative Models};
\node[node] (c13) at (0,-7.2)  {13. GANs and\\Variants};
\node[todo] (c14) at (4,-7.2)  {14. Financial\\Generative Modelling};
...
\medskip
Solid boxes are written. Dashed italic boxes are planned and not yet drafted;
their numbers are reserved so that cross-references remain stable as the book is
completed. The critical path this book exists to support runs
