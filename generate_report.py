...
Output: reports/report_YYYY-MM-DD.pdf
"""
...
\textbf{Procedure} (Engle, 1982\tcite{10}):
\begin{enumerate}
...
\purplebox{2.\ Hurst Exponent --- Long Memory (with roughness caveat)}{%
\textbf{What it captures:}\ whether $|$returns$|$ have the slow ACF decay (long memory) that real financial series exhibit.

The Hurst exponent $H$ characterises the scaling of the rescaled range (R/S estimator, Hurst 1951\tcite{23}; Mandelbrot \& Wallis 1969\tcite{24}):
\begin{equation*}
  \E\!\left[\frac{R_n}{S_n}\right] \sim c \cdot n^H
\end{equation*}
\textbf{Applied to:}\ $|r_t|$ (absolute returns), not raw $r_t$.
Rationale: R/S on raw returns recovers $H\approx0.5$ because sign flips dominate;
Ding, Granger \& Engle (1993)\tcite{8} show that long memory lives in $|r_t|$ and $r_t^2$.

\textbf{Interpretation on daily $|r_t|$:}
\begin{itemize}
  \item $H > 0.5$: long memory (Ding/Granger/Engle 1993\tcite{8}; Lobato \& Savin 1998\tcite{22})\tcite{22}.
  \item $H = 0.5$: i.i.d.\ (no memory).
  \item $H < 0.5$: anti-persistent.
\end{itemize}

\textbf{Debate --- rough volatility:}\ Gatheral, Jaisson \& Rosenbaum (2018)\tcite{17} estimate $H \approx 0.1$ on realised variance at \emph{intraday} time-scales using the variogram estimator --- not R/S, not daily data.
Cont \& Das (2024)\tcite{18} show the two camps are not contradictory: R/S on daily $|r_t|$ and the variogram on intraday RV operate at different scales and capture different phenomena. We follow the daily long-memory tradition.

\textbf{Metric:}\ $|H_\text{real} - H_\text{syn}|$, with $H$ estimated on $|r_t|$ by R/S.}

\columnbreak

\skybox{3.\ Energy Distance --- A Proper Distributional Metric}{%
The energy distance between distributions $P$ and $Q$ (Sz\'{e}kely \& Rizzo, 2004\tcite{11}):
\begin{equation*}
  E(P,Q) = 2\,\E\norm{X - Y} - \E\norm{X - X'} - \E\norm{Y - Y'}
\end{equation*}
...
\tealbox{4.\ Discriminative AUC --- Calibrated Against Empirical Null}{%
\textbf{Procedure} (TSGBench, Ang et al., 2023\tcite{15}):
\begin{enumerate}
  \item Extract 20-day rolling-window features from real and synthetic (mean, std, mean-abs, mean-sq).
  \item Label real windows 0, synthetic windows 1; train logistic regression; report AUC via 5-fold CV.
  \item \textbf{Empirical null calibration:} split the real series in half 20 times (random permutation), compute AUC(real$_A$, real$_B$) each time. This gives the null distribution for a ``perfect'' generator.
  \item Report observed AUC as a $z$-score: $z = (\text{AUC}_\text{obs} - \bar{\mu}_\text{null}) / \sigma_\text{null}$.
\end{enumerate}
\textbf{Why:} null AUC (no-shuffle CV) has std $\approx 0.08$ --- an observed AUC of 0.62 is entirely consistent with perfect generation. The $z$-score turns an uninterpretable number into a calibrated signal. \textbf{Note:} do not add \texttt{shuffle=True} to the CV; that pushes the null to $0.584 \pm 0.048$ (genuine leakage from overlapping windows).

$z \approx 0$: indistinguishable from same-distribution splits. $z \gg 0$: GAN fails to match the real distribution.}
...
\skybox{Composite Model Comparison Score}{%
\texttt{evaluate\_multiple\_models()} ranks all three GANs on each metric and computes a composite average rank across:
...
\midrule
\texttt{wasserstein}        & distributional distance \\
\texttt{energy\_distance}   & proper metric (Sz\'{e}kely 2004) \\
\texttt{quantile\_mse}      & tail-quantile fit \\
...
\smallskip
\textbf{Excluded:}\ \texttt{mse}, \texttt{mae} (pointwise --- no temporal alignment), \texttt{ks\_pvalue} (anti-conservative under serial dependence).}
...
\smallskip
\textbf{Stage 3 — Evaluation:}\ Generated synthetic series are compared with real test-set returns on 8 stylized-fact metrics (see page 10). Walk-forward re-trains each model on 5 rolling folds to verify temporal robustness. Final output: a ranked table (composite score) and fold-level mean\,$\pm$\,std.}
...
\begin{enumerate}
  ...
  \item \textbf{Stylized-fact-first evaluation} --- most published benchmarks use MSE. This pipeline uses 8 financially meaningful metrics (Wasserstein, energy distance, Hurst, ARCH-LM, discriminative AUC, tail index, ACF MAE, QMSE).
\end{enumerate}}
...
\textbf{Contribution 1:}\ First multi-architecture GAN benchmark (TimeGAN + QuantGAN + FinGAN) on BRICS emerging-market data with per-market training and walk-forward temporal validation --- 15 models, 5 markets, 8 stylized-fact metrics.
