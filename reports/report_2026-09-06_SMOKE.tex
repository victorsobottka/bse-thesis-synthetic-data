...
{\normalsize seeds=42\quad markets=BOVESPA\quad folds=2\quad generator updates=1,000}
}}
\end{center}
\vspace{4pt}
\vspace*{2.0cm}
...
\begin{tabular}{@{}ll@{}}
Generated & 2026-09-05T20:24:20 (git \texttt{276113a976}) \\
Seeds & 42 \\
Markets & BOVESPA \\
Walk-forward folds & 2 \\
Generator updates & 1,000 \\
Software & Python 3.12.3 $\cdot$ PyTorch 2.13.0+cu130 \\
CUDA & yes (NVIDIA GeForce RTX 3050 Laptop GPU) \\
Platform & Linux-7.0.0-30-generic-x86\_64-with-glibc2.39\\
\end{tabular}
\end{tcolorbox}
...
\midrule
QuantGAN & \textbf{1.5714} & 1.4286 & 1.7143 & 1.5714 \\
FinGAN & 1.9286 & 1.8571 & 2.0000 & 1.9286 \\
TimeGAN & 2.5000 & 2.7143 & 2.2857 & 2.5000 \\
CONTROL: shuffled real & --- & --- & --- & --- \\
\bottomrule
\end{tabular}}

\smallskip
The best composite score is 1.571.}
...
Rankings are computed among the generators present in \texttt{overall\_performance.csv} for this run (markets: BOVESPA). The shuffled control is appended as a reference line with NaN ranks, shown as em-dashes.
...
\textbf{Walk-forward validation.} 2 rolling fold(s) per market enable mean\,$\pm$\,std across folds rather than a single metric estimate. Budgets are equalised on generator updates across all three models (Yoon et al.'s four-phase pre-training is reported separately), so TimeGAN's temporal ranking reflects its architecture at parity, not an unequal training budget.
...
\midrule
FinGAN & 0.0017 $\pm$ 0.0011 & 0.0174 $\pm$ 0.0151 & 0.6052 $\pm$ 0.1534 \\
QuantGAN & 0.0018 $\pm$ 0.0004 & 0.0285 $\pm$ 0.0209 & 0.5789 $\pm$ 0.0365 \\
TimeGAN & 0.0041 $\pm$ 0.0002 & 0.1061 $\pm$ 0.0977 & 0.8139 $\pm$ 0.0577 \\
\bottomrule
\end{tabular}}

\smallskip
{\small Mean $\pm$ sd across 2 fold(s), market BOVESPA seed 42.}}
...
FinGAN & -8.124 & 0.0117 & 0.2135 & 0.7540 & 19 & 496 \\
QuantGAN & -8.101 & 0.0198 & 0.0298 & --- & 15 & 496 \\
TimeGAN & -7.958 & 0.0137 & 0.1416 & --- & 18 & 496 \\
\bottomrule
\end{tabular}}

\smallskip
{\small From \texttt{pooled\_downstream\_utility.csv} (BOVESPA, seed(s) 42). Lower QLIKE is better. An em-dash means Christoffersen's test was not computed (e.g.\ too few violations to fit the independence statistic).}
...
\clearpage\begin{center}\includegraphics[width=0.85\linewidth,height=0.82\textheight,keepaspectratio]{thesis_results/BOVESPA/seed42/BOVESPA_stylized_facts.png}\end{center}\centerline{\small\textit{Stylised facts: synthetic against real daily log returns}}\vspace{6pt}

\clearpage\begin{center}\includegraphics[width=0.85\linewidth,height=0.82\textheight,keepaspectratio]{thesis_results/BOVESPA/seed42/BOVESPA_metrics_heatmap.png}\end{center}\centerline{\small\textit{Normalised metrics heatmap}}\vspace{6pt}

\clearpage\begin{center}\includegraphics[width=0.85\linewidth,height=0.82\textheight,keepaspectratio]{thesis_results/BOVESPA/seed42/BOVESPA_rank_comparison.png}\end{center}\centerline{\small\textit{Average rank comparison}}\vspace{6pt}
...
Temporal 80/10/10 split (no shuffle). The 80\% training portion feeds GAN training; the 10\% test portion is the evaluation target. Walk-forward validation uses 2 rolling fold(s) within the test portion; each fold retrains the model from scratch on the fold's training segment.
...
\midrule
FinGAN & 1,000 & 5,000 & 37.0 & 55,401 \\
QuantGAN & 1,000 & 5,000 & 174.3 & 276,481 \\
TimeGAN & 1,000 & 1,000 & 19.5 & 11,400 \\
\bottomrule
\end{tabular}}
