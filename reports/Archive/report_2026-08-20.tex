...
\begin{itemize}
  ...
  \item Walk-forward evaluation: 5 rolling folds on the test set (CTBench protocol); model retrained each fold
\end{itemize}}
...
\auditrow{FIXED}{Amber}{ABg}%
  ...
  {The \texttt{evaluate\_multiple\_models()} composite ranking (used to compare model quality) included \texttt{mse} and \texttt{mae} --- meaningless as explained above (no temporal alignment between independently generated series). \textbf{Fix:} removed from \texttt{score\_cols}; replaced with \texttt{energy\_distance}, \texttt{quantile\_mse}, \texttt{hurst\_diff}, \texttt{arch\_pvalue\_diff}, and \texttt{discriminative\_auc}.}
...
\begin{enumerate}
  \item Pool the test-set series across all 5 BRICS markets.
  \item Divide into 5 rolling folds; each fold's origin advances by one test-period length.
  \item For each fold: retrain the model from scratch on the fold's training portion.
...
\textbf{Output:}\ \texttt{thesis\_results/walk\_forward/\{Model\}\_walk\_forward.csv}}
...
%% ─ Training nodes ───────────────────────────────────────────────────────────
\node[sp] (poo)  at (12.5,  0.8) {Window Pool\\128 steps};
...
\smallskip
\textbf{Stage 2 — Training:}\ Each 80\,\% training file is sliced into 128-step windows and all windows pooled together (5 markets $\times$ sliding windows). Three GAN families train on this pool; temporal order is preserved \emph{within} each window.
...
\begin{enumerate}
  \item \textbf{Multi-market pooling} --- rather than 15 separate models (3 GANs $\times$ 5 markets), one model per GAN learns universal BRICS stylized facts, enabling true cross-market generalisation (an open research question in the synthetic-finance literature).
...
\textbf{Contribution 1:}\ First multi-architecture GAN benchmark (TimeGAN + QuantGAN + FinGAN) on pooled BRICS data, with walk-forward temporal validation.
...
\smallskip
\textbf{Sliding window:}\ from a 1\,000-day series we get 1\,000\,$-$\,128\,$+$\,1\,$=$\,873 windows. Pooled across 5 BRICS markets: $\approx$4\,365 windows total. This is the training set.
...
\textbf{Why:}\ a single train/test split gives exactly one metric score. Is this score lucky or representative? Walk-forward gives 5 scores, enabling mean\,$\pm$\,std --- the standard robustness evidence required for time-series papers (Tashman 2000\tcite{13}; Bergmeir \& Ben\'{\i}tez 2012\tcite{14}). Our implementation: 5 folds, model retrained from scratch each fold, using pooled BRICS test data.}
