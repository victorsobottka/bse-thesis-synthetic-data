...
\textbf{The headline.} Across 5 markets $\times$ 3 seeds, GJR-GARCH finishes first on mean composite rank and leads the temporal family, where the econometric models take 9 of 15 per-market-seed wins. The per-cell composite count runs the other way --- gradient 10 of 15 against econometric 5 --- so the econometric advantage is in the mean rank and in the dynamics, not in winning the most cells. The margin at the top is thin: CNN-WGAN-GP trails by 0.081 on composite (2.605 against 2.524). This is not an argument that GANs do not work: CNN-WGAN-GP leads the field on \texttt{fidelity\_rank}, and the gradient family takes 9 of 15 fidelity wins. It is an argument that the property the deep models learn best --- the shape of the marginal distribution --- is the property that is cheapest to reproduce and least useful downstream, and that on the dynamics the 6-parameter GJR-GARCH still holds the field.
...
\smallskip
Composite wins: CNN-WGAN-GP 6, GARCH 3, GJR-GARCH 2, QuantGAN 2, TimeGAN 2. The econometric family takes 5 of 15, the gradient family 10.

Temporal wins: GJR-GARCH 6, GARCH 3, CNN-WGAN-GP 2, QuantGAN 2, TimeGAN 2 --- econometric 9 of 15.

Fidelity wins: CNN-WGAN-GP 5, GARCH 3, GJR-GARCH 3, QuantGAN 3, TimeGAN 1 --- gradient 9 of 15.
...
\smallskip
The best-ranked model in this benchmark, GJR-GARCH, has 6 parameters and is fitted in 0.018\,s. The largest, QuantGAN, has 276,481 --- a factor of 46,080 --- and takes 481\,s, a factor of 27,184 in wall-clock. It ranks below the smaller model on composite, on fidelity and on temporal.
...
\smallskip
On \textbf{ACF$(r^2)$ MAE} --- the canonical volatility-clustering statistic --- the shuffled control beats \emph{all 3 GANs}, and CNN-WGAN-GP, GARCH, QuantGAN, TimeGAN in all. Only GJR-GARCH clears it.

On \textbf{ACF$(|r|)$ MAE} the control beats QuantGAN, TimeGAN; CNN-WGAN-GP, GARCH, GJR-GARCH clear it.

On \textbf{ACF$(r)$ MAE} the control beats GARCH, QuantGAN, TimeGAN, and the models that do clear it do so by 0.0003 to 0.0006 --- margins on the fourth decimal.
...
\smallskip
\textbf{These metrics are not permutation-invariant.} They are functions of the ordering, and on every row of Table~3 at least one model clears the control --- as few as 1 of 5 on ACF$(r^2)$ MAE. A metric that can make that separation is measuring temporal structure. It is doing its job.
...
\smallskip
Measured against that null, 3 of 5 models sit more than 1.96 null sd away (TimeGAN, QuantGAN, GARCH). The remainder (CNN-WGAN-GP, GJR-GARCH) lie within 1.96 null sd on the fold-averaged mean and cannot, on this metric alone, be distinguished from a perfect generator at the 5\% level. 5 of 5 mean AUCs lie above the null mean. Ranking is on $|\text{AUC}-0.5|$, never on the raw value: under an ascending rank on raw AUC an anti-predictive classifier at $0.30$ would outrank an indistinguishable one at $0.50$, which inverts the meaning of the metric.
...
\smallskip
AUC falls as fold training length grows: 0.811 at fold 0 (training on 810--837 points) against 0.664 at fold 4 (4,050--4,165). The decline is \textbf{not monotonic}: fold(s) 2, 4 rise above the fold before. We report it as a trend, not a monotone relationship, because the folds do not support the stronger claim.
...
\auditrow{FIXED}{Amber}{ABg}%
  ...
  {12 of 180 econometric fits in this run reached persistence within $10^{-6}$ of 1 (NIFTY50 fold 0 GARCH; SHANGHAI main GARCH; SHANGHAI fold 3 GARCH; SHANGHAI fold 3 GJR-GARCH) and were refit under persistence $\le1-10^{-4}$; output was generated from the constrained parameters and both parameter sets are stored (Results, Table~8). An earlier version raised and stopped the run on such fits, which would have kept SHANGHAI from completing on any seed; it was replaced because stopping the benchmark is not a treatment of the fit. The post-generation guard still raises outside $1/10$--$10\times$ the training sd.}
...
\textit{Failure mode, measured:} these metrics are ordering-sensitive by construction, yet 4 of the 5 generators still lose to the shuffled control on at least one of them. Averaged over all 15 market-seeds the control scores 0.0570 on ACF$(r^2)$ MAE, beating every GAN, and only GJR-GARCH clears it (Results, Table~3). Reported as a result about the generators, not as grounds for reclassifying the metric.
