...
\item \textbf{Every generator's AUC lies above the real-versus-real null, but no result reaches the 5\,\% level.} The AUC quoted here is the walk-forward one (Section~\ref{sec:walkforward}), because it is scored out of time: a classifier-based indistinguishability test\tcite{4} compares a freshly trained model with the real block that follows its training data, in every fold. 5 of 5 fold-averaged AUCs are above the null mean, but the null has only 15 draws, so the smallest attainable Monte Carlo $p$-value is $1/16=0.0625$. AUC falls from 0.811 to 0.664 as the training window grows; the decline is not monotonic, and training length and calendar period grow together.
...
Two counts recur and are not the same. The \emph{per-market metrics} are the 19 columns of each \texttt{<market>\_metrics.csv}: seven fidelity, seven temporal and five descriptive. Of these, 9 are permutation-invariant --- the seven fidelity metrics plus two descriptive ones, \texttt{skewness\_diff} and \texttt{kurtosis\_diff}. Seven of those 9 are ranked; the two descriptive ones are not. Separately, the pipeline's \texttt{DESCRIPTIVE\_COLS} has seven members: those two, \texttt{arch\_pvalue\_diff}, \texttt{arch\_stat\_diff} and \texttt{discriminative\_auc\_raw}, which are among the five in the per-market files, and \texttt{var\_coverage\_error} and \texttt{garch\_persistence\_diff}, which come from the pooled downstream backtest and appear in no per-market file. Every ``$k$ of $N$'' below says which of these it counts.
...
\caption{Every quantity the pipeline computes: what it measures, how it is computed, where it comes from, and whether it is ranked. The first 19 rows (fidelity, temporal, and five descriptive) are the per-market metrics of \texttt{<market>\_metrics.csv}; the last five (downstream utility) are computed once per seed on the pooled sample. ``This work'' marks a metric defined for this study rather than taken from the literature.}\label{tab:metrics}\\
\toprule
...
\emph{Track A}, on the left, is the fixed-split evaluation. Each model is fitted once on Train and generates a series of Test length (\emph{Synthetic returns}), which is set against the real Test series (the dotted edge) on the 19 per-market metrics: seven ranked fidelity metrics, seven ranked temporal metrics and five descriptive ones that are reported but not ranked. Only the fidelity and temporal groups feed the \emph{Composite rank}. This is Table~\ref{tab:ranking}: 15 evaluations in total, one training per model per evaluation, 5~models~$\times$~15~evaluations~=~75 trainings.
...
\begin{table}[htbp]
\centering
\caption{Mean ranks over 15 market--seed evaluations; lower is better. \emph{Wins}: evaluations in which the model has the lowest composite rank --- the gradient-trained family takes 10 of 15 composite wins and the econometric family 5. \emph{Fit}: mean wall-clock seconds to train or fit one model; GARCH and GJR-GARCH fit roughly 21,000$\times$ and 27,000$\times$ faster than QuantGAN respectively. The shuffled control is scored on every metric but does not compete for rank.}
\label{tab:ranking}
\begin{tabular}{@{}llccccr@{}}
\toprule
Model & Family & Composite & Fidelity & Temporal & Wins & Fit (s) \\
\midrule
GJR-GARCH & econometric & \textbf{2.524} & 2.667 & 2.381 & 2\,/\,15 & 0.018 \\
CNN-WGAN-GP & gradient & 2.605 & 2.429 & 2.781 & 6\,/\,15 & 180 \\
GARCH & econometric & 2.967 & 3.152 & 2.781 & 3\,/\,15 & 0.023 \\
...
\paragraph{Why this measure} A classifier-based discriminative score is the indistinguishability test of TimeGAN\tcite{4}, adopted by CTBench\tcite{16}. Walk-forward evaluation keeps test data strictly after training data, as time-series evaluation requires (Tashman\tcite{13}; Bergmeir \& Ben\'{\i}tez\tcite{14}); this is the AUC quoted in the Highlights and throughout the text, not the fixed-split \texttt{discriminative\_auc\_dist} of Table~\ref{tab:metrics}. The null is not exactly 0.5: over $B=15$ real-versus-real half-splits it was $\mu_0=0.506$ with $\sigma_0=0.084$, a prior measurement. $\sigma_0$ is the standard deviation of individual null draws, not the standard error of the null mean ($\sigma_0/\sqrt{B}=0.022$). Each AUC is read as a distance from that null in units of $\sigma_0$.

\paragraph{What this result tells us} \emph{Significance.} 5 of 5 fold-averaged AUCs lie above the null mean; Table~\ref{tab:auc} reports each as a distance from the null mean in units of $\sigma_0$ (the $z$ column), not as a test at a fixed level. Whether an AUC is unusually high for real data is a Monte Carlo question: against $B$ null draws the $p$-value of an observed AUC $a$ is
\[ p=\frac{1+\#\{\text{null}\ge a\}}{B+1}. \]
With $B=15$ its smallest attainable value is $1/16=0.0625$, so \emph{no result reaches the 5\,\% level}, however far above the null it lies. The individual null draws were not kept in this run's artifacts, so no per-model $p$-value is tabulated; the floor holds whatever they were. The draws are also not independent: they are half-splits of the same realized series, so the null describes variability conditional on this path, not across independent market histories. A normal-theory threshold such as $z>1.96$ is not used, because it would claim a significance the null cannot support. On multiple comparisons: five models each tested at 5\,\% give 0.25 expected false flags, and a Bonferroni correction would use $z=2.58$, an AUC threshold of $\mu_0+2.58\,\sigma_0=0.723$ (normal approximation).
...
The ranking agrees. TimeGAN is the worst of the five on \textbf{all 7 of 7 temporal metrics}, on 5 of 7 fidelity metrics, and on 14 of the 19 per-market metrics overall. It is \emph{not} worst on std\_diff (QuantGAN), skewness\_diff (QuantGAN), quantile\_mse (QuantGAN), arch\_pvalue\_diff (GARCH), arch\_stat\_diff (QuantGAN) --- the model in brackets is worse there --- so ``worst on every metric'' would be an overstatement, and the scope is stated instead.
